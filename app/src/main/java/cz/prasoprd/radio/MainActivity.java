    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(() -> {
            while (true) {
                try {
                    Thread.sleep(10);
                    textView.post(() -> textView.setText(DateTimeFormatter.ofPattern("h:mm:ss a - EEEE - d/M/yyyy").format(LocalDateTime.now())));
                    textView2.post(() -> textView2.setText("Volume: " + seekBar.getProgress()));
        ...
        ((Button) findViewById(R.id.button2)).setOnClickListener((View.OnClickListener) v -> {
            Radio.play(v,"https://streams.ilovemusic.de/iloveradio2.mp3");
            createNotification("I ♥ Dance");
        });

        ((Button) findViewById(R.id.button3)).setOnClickListener((View.OnClickListener) v -> {
            Radio.play(v,"https://streams.ilovemusic.de/iloveradio3.mp3");
            createNotification("I ♥ HipHop");
        });

        ((Button) findViewById(R.id.button4)).setOnClickListener((View.OnClickListener) v -> {
            Radio.play(v,"https://streams.ilovemusic.de/iloveradio5.mp3");
            createNotification("I ♥ Mashup");
        });

        ((Button) findViewById(R.id.button6)).setOnClickListener((View.OnClickListener) v -> {
            Radio.play(v,"https://stream.laut.fm/cokeradio");
            createNotification("Coke radio");
        });

        ((Button) findViewById(R.id.button7)).setOnClickListener((View.OnClickListener) v -> {
            Radio.stop(v);
            removeNotification();
        });
    }
